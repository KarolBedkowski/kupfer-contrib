...
class EncryptText(Action):
    ...
    def activate_multiple(self, objects, iobjects):
        ...
        self.async_token, task = _create_gpg_task(cli, text,
                                                  self._finish_callback)
        return task
...
class SignEncryptText(Action):
    ...
    def get_description(self):
        return _(
            "Sign text by default key and encrypt it for selected recipients")
...
class VerifySignature(Action):
    ...
    def activate(self, leaf):
        ...
        uiutils.show_text_result(
            stdout or stderr, title=_("Verification Result"))
