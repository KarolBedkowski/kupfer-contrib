...
class LinkToPublicAndGetUrl(Action):
    ...
    def activate(self, leaf):
        dest = os.path.join(self._dropbox_public,
                            os.path.basename(leaf.object))
        if os.path.exists(dest):
            ...
            try:
                ...
                return TextLeaf(
                    _("Error creating link to file: %s") % str(err))
        if os.path.exists(dest):
            try:
    ...
    def get_description(self):
        return _(
            'Create link to the file in Dropbox/Public and get public URL')
